...
test('Trim False', function () {
    assert.equal(moment.duration(1, 'seconds').format('m s', { trim: false }), '0 1');
    assert.equal(moment.duration(1, 'minutes').format('h m s', { trim: false }), '0 1 0');
});
...
test('Left Trimmed First Token Length', function () {
    assert.equal(moment.duration(1, 'seconds').format('mm ss'), '01');
...
test('Positive Precision', function () {
    ...
    assert.equal(moment.duration(40, 'seconds').format('m', 5), '0.66666');
    assert.equal(moment.duration(45, 'seconds').format('m', 1), '0.7');
    assert.equal(moment.duration(59, 'seconds').format('m', 0), '0');
    assert.equal(moment.duration(59, 'seconds').format('m'), '0');
});
...
test('All Moment Tokens', function () {
    ...
    assert.equal(moment.duration(100000000013, 'ms').format('y[y] M[mo] w[w] d[d] h[h] m[m] s[s] S[ms]'), '3y 2mo 0w 2d 9h 46m 40s 13ms');
});
...
test('Output To Greater Units', function () {
    assert.equal(moment.duration(1, 'milliseconds').format('y', 15), '0.000000000031709');
    assert.equal(moment.duration(1, 'milliseconds').format('M', 13), '0.0000000003858');
    assert.equal(moment.duration(1, 'milliseconds').format('w', 12), '0.000000001653');
    assert.equal(moment.duration(1, 'milliseconds').format('d', 11), '0.00000001157');
    assert.equal(moment.duration(1, 'milliseconds').format('h', 10), '0.0000002777');
    assert.equal(moment.duration(1, 'milliseconds').format('m', 7), '0.0000166');
    assert.equal(moment.duration(1, 'milliseconds').format('s', 3), '0.001');
    assert.equal(moment.duration(1, 'milliseconds').format('S'), '1');
});

test('Custom Token Types List', function () {
    assert.equal(moment.duration(12345, 'seconds').format('d [days] m [minutes] h [(hours is not a token type now)]', {
...
test('Custom Template Function', function () {
    assert.equal(moment.duration(100, 'days').format(function () {
...
test('Negative Durations', function () {
    ...
    assert.equal(moment.duration(-1, 'milliseconds').format('S'), '-1');
    assert.equal(moment.duration(-1, 'years').format('s'), '-31536000');
    assert.equal(moment.duration(-1, 'seconds').format('y', 15), '-0.000000031709791');
    assert.equal(moment.duration(-65, 'seconds').format('m:ss'), '-1:05');
    assert.equal(moment.duration(-65, 'seconds').format('m:ss', 2), '-1:05.00');
    assert.equal(moment.duration(-65.667, 'seconds').format('m:ss', 2), '-1:05.66');
    assert.equal(moment.duration(-65.667, 'days').format('d', 2), '-65.66');
